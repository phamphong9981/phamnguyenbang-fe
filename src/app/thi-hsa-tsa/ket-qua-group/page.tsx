...
import { useExamGroupResult } from '@/hooks/useExam';
...
function ExamGroupResultContent() {
    ...
    const { data: examResult, isLoading, error } = useExamGroupResult(groupId);

    if (isLoading) {
        return <ExamGroupResultLoading />;
    }
                ...
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="bg-white rounded-2xl shadow-lg overflow-hidden mb-8">
                        <div className="bg-gradient-to-r from-green-500 to-emerald-600 px-8 py-6 text-white">
                            <div className="text-center">
                                <h2 className="text-3xl font-bold mb-2">Tổng kết</h2>
                                <div className="text-6xl font-bold mb-2">{examResult.percentage}%</div>
                                <div className="text-2xl">
                                    {examResult.totalPoint}/{examResult.maxPoints} điểm
                                </div>
                                <p className="mt-4 text-xl text-green-50">{examResult.message}</p>
                            </div>
                        </div>
                    </div>
...
export default function ExamGroupResultPage() {
    return (
        <Suspense fallback={<ExamGroupResultLoading />}>
            <ExamGroupResultContent />
        </Suspense>
    );
}
